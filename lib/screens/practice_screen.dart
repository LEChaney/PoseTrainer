...
import '../services/wasm_drawing_service.dart'
    if (dart.library.io) '../services/wasm_drawing_service_stub.dart';
...
class _PracticeScreenState extends State<PracticeScreen>
  ...
  // Platform-conditional drawing backend
  late final BrushEngine? engine; // null on web when using WASM
  WasmDrawingService? _wasmService; // only on web
  bool _wasmEventLoopStarted =
      false; // track if we've started the WASM event loop
  final bool _useWasm =
      kIsWeb; // Use WASM canvas on web, Flutter canvas elsewhere
  ...
  void initState() {
    ...
    // Load saved brush settings
    final savedSettings = BrushSettingsService.load();
    debugLog(
      'Loaded brush settings: ${savedSettings.toString()}',
      tag: 'Practice',
    );
    ...
    // Initialize platform-specific drawing backend
    if (_useWasm) {
      // Web: Use Rust WASM canvas
      // Constructor immediately generates viewId and registers platform view
      _wasmService = WasmDrawingService();
      debugLog(
        'WASM service created, viewId: ${_wasmService!.viewId}',
        tag: 'Practice',
      );

      // Initialize WASM module asynchronously after build
      // This must complete before startEventLoop() can be called
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        try {
          await _wasmService!.initialize(_baseWidthPx, _baseHeightPx);
          debugLog('WASM module loaded', tag: 'Practice');

          // Mark as ready for event loop to start
          if (mounted) {
            setState(
              () {},
            ); // Trigger rebuild so _buildCanvasArea can start event loop
          }
        } catch (e) {
          debugLog('Failed to initialize WASM module: $e', tag: 'Practice');
        }
      });
      engine = null; // No BrushEngine on web
    } else {
      // Native: Use Flutter BrushEngine with saved settings applied
      final params = const BrushParams().copyWith(
        runtimeSizeScale: savedSettings.sizeScale,
        runtimeFlowScale: savedSettings.flowScale,
        hardness: savedSettings.hardness,
      );
      engine = BrushEngine(params, profiler: _profiler)..prepare();
      debugLog(
        'Applied saved settings to Flutter engine: size=${savedSettings.sizeScale}, flow=${savedSettings.flowScale}, hardness=${savedSettings.hardness}',
        tag: 'Practice',
      );
      _wasmService = null;
    }
    ...
    // On web with WASM, the ticker is not needed (WASM handles its own rendering)
    if (!_useWasm) {
      _ticker = createTicker(_onFrame)..start();
    } else {
      // Create a no-op ticker for WASM mode to satisfy the late field
  ...
  void _onFrame(Duration _) {
    ...
    // Only used for Flutter engine, not WASM
  ...
  Future<void> _finish() async {
    ...
    if (_useWasm && _wasmService != null) {
      // Export from WASM canvas
      finalImg = await _wasmService!.exportCanvas();
    } else if (engine != null) {
      // Export from Flutter engine
      await engine!.bakeLiveToTiles();
      finalImg = await engine!.renderFull(_baseWidthPx, _baseHeightPx);
    } else {
      debugLog('No drawing backend available for export', tag: 'Practice');
      return;
    }
  ...
  void dispose() {
    ...
    // Dispose drawing backend
    if (_useWasm) {
      _wasmService?.dispose();
    } else {
  ...
  Widget _buildCanvasArea(double dpr, Size canvasLogical) {
    // On web with WASM, embed the WASM canvas using HtmlElementView
    // This allows Flutter's layout system to properly position and size the canvas
    if (_useWasm && _wasmService != null) {
      // Start the event loop after the widget is built (only once)
      // Only start if WASM module is fully initialized
      if (!_wasmEventLoopStarted && _wasmService!.isFullyInitialized) {
        _wasmEventLoopStarted = true;
        WidgetsBinding.instance.addPostFrameCallback((_) async {
          try {
            // Apply saved brush settings BEFORE starting event loop
            // Settings go into Rust global state which persists across app reinit
            final savedSettings = BrushSettingsService.load();
            ...
            // Now start event loop - App will be created with global brush params
            await _wasmService!.startEventLoop();
            debugLog(
              'WASM event loop started with persisted brush settings',
              tag: 'Practice',
            );
          } catch (e) {
            debugLog('Failed to start WASM event loop: $e', tag: 'Practice');
          }
        });
      }

      return HtmlElementView(viewType: _wasmService!.viewId);
    }

    // Native platforms: use Flutter canvas
    return _CanvasArea(
      engine: engine!,
      pressure: _pressure,
      nowMs: _nowMs,
...
class _BrushControls extends StatefulWidget {
  final BrushEngine? engine; // null when using WASM
  final WasmDrawingService? wasmService; // null when using Flutter engine
  final bool useWasm;

  const _BrushControls({
    required this.engine,
    required this.wasmService,
...
class _BrushControlsState extends State<_BrushControls> {
  ...
  void initState() {
    super.initState();
    // Load saved settings - these are authoritative
    final savedSettings = BrushSettingsService.load();
    _size = savedSettings.sizeScale;
    _flow = savedSettings.flowScale;
    _hardness = savedSettings.hardness;

    debugLog(
      'Brush controls initialized from saved settings: size=$_size, flow=$_flow, hardness=$_hardness',
      tag: 'BrushControls',
    );
  }

  void _updateBrushParam(String param, double value) {
    // Update local state
    setState(() {
      switch (param) {
        case 'size':
        ...
        case 'hardness':
          _hardness = value;
          // Don't auto-close hardness slider on value change
          break;
      }
    });

    // Save to persistent storage
    final currentSettings = BrushSettings(
      sizeScale: _size,
      flowScale: _flow,
      hardness: _hardness,
    );
    BrushSettingsService.save(currentSettings);

    // Update drawing backend
    if (widget.useWasm && widget.wasmService != null) {
      // Update WASM backend
      switch (param) {
        case 'size':
          // Convert normalized 0-1 to pixel size (matching Flutter's maxSizePx = 100)
          widget.wasmService!.setBrushSize(value * 100);
          break;
        case 'flow':
          widget.wasmService!.setBrushFlow(value);
          break;
        case 'hardness':
          widget.wasmService!.setBrushHardness(value);
          break;
      }
      ...
      switch (param) {
        case 'size':
          widget.engine!.setSizeScale(value);
          break;
        case 'flow':
          widget.engine!.setFlowScale(value);
          break;
        case 'hardness':
          widget.engine!.setHardness(value);
          break;
      }
    }
  }
  ...
  Widget _buildEdgeOverlay(BuildContext context) {
    return _EdgeBrushAdjust(
      ...
      onSizeChanged: (v) {
        ...
        _updateBrushParam('size', _size);
      },
      onFlowChanged: (v) {
        setState(() => _flow = v.clamp(0.0, 1.0));
        _updateBrushParam('flow', _flow);
      },
    );
  }
...
/// Uses quadratic scaling to give more precision at lower values.
...
class _VerticalSliderState extends State<_VerticalSlider> {
  ...
  // Convert actual value (0-1) to slider position (0-1) using inverse of quadratic
  // sliderPos = sqrt(value) gives more precision at lower values
  double _valueToSlider(double value) => math.sqrt(value.clamp(0.0, 1.0));

  // Convert slider position (0-1) to actual value (0-1) using quadratic
  // value = sliderPos^2 means bottom 50% of slider covers 0-0.25 of value range
  double _sliderToValue(double sliderPos) => sliderPos * sliderPos;
              ...
              Expanded(
                child: Listener(
                  ...
                  child: RotatedBox(
                    quarterTurns: 3,
                    child: Slider(
                      value: _valueToSlider(widget.value),
                      min: 0,
                      max: 1,
                      onChanged: (sliderPos) {
                        final actualValue = _sliderToValue(sliderPos);
                        widget.onChanged(actualValue);
                        if (_dragging) setState(() => _lastValue = actualValue);
                      },
                    ),
                  ),
                ),
...
class _EdgeBrushAdjustState extends State<_EdgeBrushAdjust> {
  ...
  void _beginDrag(
    ...
    setState(() {
      ...
      _startSize = _size;
      _startFlow = _flow;
      // Compute fraction at touch in slider space (sqrt of value)
  ...
  void _updateDrag(Offset pos) {
    ...
    // Use quadratic scaling: work in slider space (sqrt of value space)
    final localDy = (pos.dy - _trackTop).clamp(0.0, _trackHeight);
    final currSliderFrac = (1.0 - (localDy / _trackHeight)).clamp(0.0, 1.0);
    final baseSize = _startSize ?? _size;
    final baseFlow = _startFlow ?? _flow;
    final touchSliderFrac = _touchFracStart ?? currSliderFrac;
    final sliderDelta = currSliderFrac - touchSliderFrac;

    if (_adjustSize) {
      // Convert to slider space, apply delta, convert back to value space
      final baseSizeSlider = _valueToSlider(baseSize);
      final newSizeSlider = (baseSizeSlider + sliderDelta).clamp(0.0, 1.0);
      _size = _sliderToValue(newSizeSlider);
      widget.onSizeChanged(_size);
      _bubbleText = '${(_size * 100).round()}%';
    } else {
      final baseFlowSlider = _valueToSlider(baseFlow);
      final newFlowSlider = (baseFlowSlider + sliderDelta).clamp(0.0, 1.0);
      _flow = _sliderToValue(newFlowSlider);
                            ...
                            child: Slider(
                              ...
                              onChanged: (sliderPos) {
                                ...
                                setState(() {
                                  _size = actualValue;
                                  _bubbleText =
                                      '${(actualValue * 100).round()}%';
                                });
                                widget.onSizeChanged(actualValue);
                              },
                          ...
                          child: RotatedBox(
                            ...
                            child: Slider(
                              ...
                              onChanged: (sliderPos) {
                                ...
                                widget.onFlowChanged(actualValue);
                              },
                              onChangeEnd: (_) => setState(() {
                                _bubbleText = null;
                                _bubblePos = null;
